...
function parseTextLiteral(text) {
  ...
  parsedFn['exp'] = text; // Needed to pretend to be $interpolate for tests copied from interpolateSpec.libs
...
InterpolationParts.prototype.toParsedFn = function toParsedFn(mustHaveExpression, originalText) {
  ...
  parsedFn['exp'] = originalText; // Needed to pretend to be $interpolate for tests copied from interpolateSpec.libs
...
MessageFormatParser.prototype.ruleEndMustache = function ruleEndMustache() {
  ...
  if (this.parsedFn == null) {
    ...
    this.parsedFn['exp'] = this.expressionFn['exp']; // Needed to pretend to be $interpolate for tests copied from interpolateSpec.libs
...
MessageFormatParser.prototype.ruleInAngularExpression = function ruleInAngularExpression() {
  ...
  if (match == null) {
    if (this.angularOperatorStack.length === 0) {
      ...
      // Needed to pretend to be $interpolate for tests copied from interpolateSpec.libs
...
var $$interpolateDecorator = ['$$messageFormat', '$delegate', function $$interpolateDecorator($$messageFormat, $interpolate) {
  if ($interpolate['startSymbol']() !== '{{' || $interpolate['endSymbol']() !== '}}') {
    throw $interpolateMinErr('nochgmustache', 'angular-message-format.libs currently does not allow you to use custom start and end symbols for interpolation.');
  }
